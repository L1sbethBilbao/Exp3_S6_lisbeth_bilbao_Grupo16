...
/**
 * Autenticacion (token del canal) y autorizacion (permisos WEB) del BFF Web.
 */
@Component
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response,
        ...
        if (!tokenEsperado.equals(token)) {
            escribirError(response, HttpServletResponse.SC_UNAUTHORIZED,
                    "Token del canal web invalido o ausente");
            return;
        }
        ...
        if (!permisos.contains("CONSULTA_COMPLETA")) {
            escribirError(response, HttpServletResponse.SC_FORBIDDEN,
                    "Canal WEB sin permiso CONSULTA_COMPLETA");
            return;
        }
